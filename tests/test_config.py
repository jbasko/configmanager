...
def test_cool_undocumented_feature():
    # In some config managers you see the concept of views, so user can create
    # proxies to config paths, but they only get evaluated when user actually
    # asks some real stuff.

    # If we keep iterators like they are that the path is not evaluated until
    # we start iterating then we can use them to provide these lazy loaded proxies.

    config = Config()

    # At this moment the item doesn't exist yet
    threads_iterator = config.iter_items(path='threads', key=None)

    # Item is declared
    config.add_schema({'threads': 1})

    # Voila!
    assert next(threads_iterator).value == 1

    #
    # This can be wrapped in a method like get_proxy(*key) below.
    # Of course, a proper proxy base class should be used which implements
    # much more than just __getattr__
    #
    def get_proxy(config, *key):
        class Proxy(object):
            def __init__(self, generator):
                self.__generator = generator
                self.__real_object = not_set

            def __get_real_object(self):
                if self.__real_object is not_set:
                    self.__real_object = next(self.__generator)
                return self.__real_object

            def __getattr__(self, item):
                return getattr(self.__get_real_object(), item)

        return Proxy(config.iter_all(path=key, key=None))

    enabled = get_proxy(config, 'uploads.enabled')
    config.add_schema({'uploads': {'enabled': True}})
